...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		int char_len = w.length;
		char[] flag  = new char[char_len];
		int flag_y = 0;
		int totalsyl = 0;
		
		for(int i = 0; i<= char_len-1 ; i++){
			if (w[i] == 'A' || w[i] == 'E' || w[i] == 'I' || w[i] == 'O' || w[i] == 'U' || w[i] == 'Y'
					|| w[i] == 'a' || w[i] == 'e' || w[i] == 'i' || w[i] == 'o' || w[i] == 'u' || w[i] == 'y'){
				flag[i] = 'y';
				flag_y++;
			}
			else flag[i] = 'n';
		...
		if(flag_y >=2){
			if(w[char_len-1] == 'e'){
				if(w[char_len-2] == 'a' || w[char_len-2] == 'e' || w[char_len-2] == 'i'|| w[char_len-2] == 'o' || w[char_len-2] == 'u' || w[char_len-2] == 'y'
						|| w[char_len-2] == 'A' || w[char_len-2] == 'E'|| w[char_len-2] == 'I'|| w[char_len-2] == 'O'|| w[char_len-2] == 'U'|| w[char_len-2] == 'Y'
						&& flag[char_len-2] =='n'){
					flag[char_len-2] ='y';
				}
				flag[char_len-1] = 'n';
	...
	public double getFleschScore()
	{
		...
		double flesch = 206.835 - 1.015 *((double)this.getNumWords()/(double)this.getNumSentences()) 
